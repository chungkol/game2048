...
class ViewController: UIViewController {
    ...
    func checkLose() ->Bool {
        for( var i = 0 ; i < 4; i++)
        {
            for(var j = 0; j < 4; j++)
            {
                if (checkValue(i + 1, j: j, value: b[i][j]) || checkValue(i - 1, j: j, value: b[i][j]) || checkValue(i, j: j + 1, value: b[i][j]) || checkValue(i, j: j - 1,  value: b[i][j])) {
                    false
                }
            }
        }
    ...
    func checkValue(i: Int, j: Int , value: Int) -> Bool
    {
        if !checkOutRanger(i, j: j) {
    ...
    func checkOutRanger(i: Int, j: Int) -> Bool{
        if (i < 0 || i > 3 || j < 0 || j > 3  ) {
            return true
        }
        return false
